...
int main(int argc,char** argr)
{
    ...
    glutInit(&argc, argr);
    glutInitDisplayMode(GLUT_SINGLE | GLUT_RGB); // mode setting

    glutInitWindowSize(500, 500); // window size

    glutInitWindowPosition(50, 50);//window position

    glutCreateWindow("Batman");//create window with label

    glutDisplayFunc(Display);// call back function to render the windows

    glutIdleFunc(Anim);
    glClearColor(1.0f, 1.0f, 1.0f, 0.0f);
    gluOrtho2D(0.0, 500.0, 0.0, 500.0);
    glutMainLoop();// All windows that have been created can now be shown, and rendering those windows is now effective
    return 0;
}
